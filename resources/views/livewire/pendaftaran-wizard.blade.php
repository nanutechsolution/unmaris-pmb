            ...
            <div class="animate-fade-in-up">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="nik" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 focus:bg-white focus:outline-none focus:shadow-neo transition-all font-medium text-unmaris-blue text-sm md:text-base">
                ...
                <div class="mt-8 flex justify-end">
                    <button wire:click="validateStep1" class="w-full md:w-auto bg-unmaris-yellow hover:bg-yellow-400 text-unmaris-blue font-black py-3 px-8 rounded-lg border-2 border-unmaris-blue shadow-neo hover:shadow-neo-hover transition-all transform uppercase tracking-wider">
                        Lanjut ke Akademik 👉
                    </button>
                </div>
            </div>
            ...
            <div class="animate-fade-in-up">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="tahun_lulus" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 focus:bg-white focus:outline-none focus:shadow-neo transition-all font-medium text-unmaris-blue text-sm md:text-base">
                    ...
                    <div class="md:col-span-2 bg-blue-50 p-4 border-2 border-unmaris-blue rounded-xl">
                        ...
                        <select wire:model.live="pilihan_prodi_1" class="w-full bg-white border-2 border-unmaris-blue rounded-lg py-3 px-4 font-bold cursor-pointer text-unmaris-blue">
                            ...
                            <!-- Menggunakan Data dari Database Lokal (Sync) -->
                        ...
                        <div class="mt-4">
                            <label class="block text-xs font-bold text-gray-500 mb-1 uppercase">Pilihan Kedua (Opsional)</label>
                            <select wire:model="pilihan_prodi_2" class="w-full bg-white border-2 border-gray-300 rounded-lg py-2 px-3 text-sm">
                ...
                <div class="mt-8 flex flex-col-reverse md:flex-row justify-between gap-3">
                    ...
                    <button wire:click="validateStep2" class="w-full md:w-auto bg-unmaris-yellow text-unmaris-blue font-black py-3 px-8 rounded-lg border-2 border-unmaris-blue shadow-neo hover:shadow-none transition-all">
                        Lanjut ke Berkas 👉
                    </button>
                </div>
            </div>
        ...
        @if($currentStep == 3)
            <div x-data="{ agreed: false }" class="animate-fade-in-up">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2 bg-yellow-100 p-6 border-4 border-yellow-500 rounded-xl relative shadow-sm">
                        ...
                        <div class="flex items-center justify-center w-full">
                            <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-yellow-500 border-dashed rounded-lg cursor-pointer bg-yellow-50 hover:bg-white transition">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    <span class="text-3xl">📁</span>
                                    <p class="mb-2 text-sm text-yellow-700 font-bold"><span class="font-black">Klik upload</span> PDF (Max 5MB)</p>
                                </div>
                                <input type="file" wire:model="file_beasiswa" class="hidden" accept=".pdf" />
                            </label>
                        </div>
                        
                        @if($file_beasiswa)
                            <p class="text-xs font-bold text-green-600 mt-2">File terpilih: {{ $file_beasiswa->getClientOriginalName() }}</p>
                        @elseif($existingFileBeasiswaPath)
                            <p class="text-xs font-bold text-green-600 mt-2">✅ Berkas sudah tersimpan.</p>
                        @endif
                        @error('file_beasiswa') <span class="text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    @endif
                    ...
                    <div class="bg-white border-2 border-unmaris-blue rounded-xl p-6 text-center hover:bg-yellow-50 transition shadow-neo group relative">
                        ...
                        <input type="file" wire:model="foto" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                        @error('foto') <span class="text-red-600 font-bold text-xs block mt-2">{{ $message }}</span> @enderror
                    ...
                    <div class="bg-white border-2 border-unmaris-blue rounded-xl p-6 text-center shadow-neo relative group">
                        ...
                        <div class="mt-2 flex justify-center">
                            @if ($ijazah)
                                <div class="flex flex-col items-center justify-center p-4 bg-blue-50 border-2 border-unmaris-blue rounded-lg"><span class="text-2xl">📄</span><span class="text-xs font-bold mt-1 text-unmaris-blue">Siap Upload</span></div>
                            @elseif($existingIjazahPath)
                                <div class="flex flex-col items-center justify-center p-4 bg-green-100 border-2 border-unmaris-blue rounded-lg"><span class="text-2xl">✅</span><span class="text-xs font-black mt-1 text-green-900">TERSIMPAN</span></div>
                            @else
                                <div class="w-32 h-32 bg-gray-200 rounded-lg border-4 border-unmaris-blue border-dashed flex items-center justify-center"><span class="text-xs text-gray-500 font-bold px-2">Klik Upload</span></div>
                            @endif
                        </div>

                        <input type="file" wire:model="ijazah" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                        @error('ijazah') <span class="text-red-600 font-bold text-xs block mt-2">{{ $message }}</span> @enderror
                ...
                <div class="mt-6 flex items-center bg-yellow-50 p-4 rounded-xl border-2 border-unmaris-blue">
                    <input type="checkbox" x-model="agreed" class="w-6 h-6 text-unmaris-blue border-2 border-unmaris-blue rounded focus:ring-0 cursor-pointer flex-shrink-0">
                ...
                <div class="mt-8 flex flex-col-reverse md:flex-row justify-between items-center gap-3">
                    ...
                    <button wire:click="submit" :disabled="!agreed" :class="{'opacity-50 cursor-not-allowed': !agreed, 'bg-unmaris-green hover:bg-green-600 hover:shadow-none': agreed}" wire:loading.attr="disabled" class="w-full md:w-auto bg-gray-300 text-white font-black py-3 px-8 rounded-lg border-2 border-unmaris-blue shadow-neo transition-all transform uppercase tracking-wider flex justify-center items-center">
