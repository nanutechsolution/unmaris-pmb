<div class="max-w-4xl mx-auto py-6 md:py-10 font-sans px-4 md:px-0" 
        ...
        <div class="relative">
            ...
            <div class="flex justify-between w-full">
                ...
                <div class="relative flex flex-col items-center group">
                    <div class="{{ $currentStep >= 1 ? 'bg-unmaris-yellow text-unmaris-blue translate-x-[-2px] translate-y-[-2px] shadow-neo' : 'bg-white text-gray-400' }} w-10 h-10 md:w-12 md:h-12 border-2 border-unmaris-blue rounded-xl flex items-center justify-center font-black text-lg md:text-xl z-10 transition-all duration-200">1</div>
                ...
                <!-- Step 2 -->
                <div class="relative flex flex-col items-center group">
                    <div class="{{ $currentStep >= 2 ? 'bg-unmaris-yellow text-unmaris-blue translate-x-[-2px] translate-y-[-2px] shadow-neo' : 'bg-white text-gray-400' }} w-10 h-10 md:w-12 md:h-12 border-2 border-unmaris-blue rounded-xl flex items-center justify-center font-black text-lg md:text-xl z-10 transition-all duration-200">2</div>
                ...
                <!-- Step 3 -->
                <div class="relative flex flex-col items-center group">
                    <div class="{{ $currentStep >= 3 ? 'bg-unmaris-yellow text-unmaris-blue translate-x-[-2px] translate-y-[-2px] shadow-neo' : 'bg-white text-gray-400' }} w-10 h-10 md:w-12 md:h-12 border-2 border-unmaris-blue rounded-xl flex items-center justify-center font-black text-lg md:text-xl z-10 transition-all duration-200">3</div>
    ...
    <div class="bg-white p-5 md:p-8 border-4 border-unmaris-blue shadow-neo-lg rounded-3xl relative overflow-hidden">
        ...
        <div class="mb-6">
            <span class="text-xs font-bold text-red-500 bg-red-50 px-2 py-1 rounded border border-red-200">(*) Wajib Diisi</span>
        </div>
        ...
        @if ($currentStep == 1)
            <div class="animate-fade-in-up space-y-6">
                ...
                <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg">
                    <p class="text-sm font-black text-red-700">PENTING: Data Nama, Tempat, dan Tanggal Lahir WAJIB SESUAI DENGAN IJAZAH.</p>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-yellow-100 p-4 border-2 border-yellow-500 rounded-lg @error('scholarship_id') has-error @enderror">
                        <label class="block text-sm font-black text-yellow-900 mb-2">Pilih Program Beasiswa *</label>
                        <select wire:model="scholarship_id" class="w-full bg-white border-2 border-yellow-600 rounded-lg py-3 px-4 font-bold text-gray-800">
                            ...
                            @foreach(\App\Models\Scholarship::where('is_active', true)->get() as $s)
                                <option value="{{ $s->id }}">{{ $s->name }} (Sisa: {{ max(0, $s->quota - $s->pendaftars_count) }})</option>
                            @endforeach
                        </select>
                        @error('scholarship_id') <span class="validation-error text-red-600 text-xs font-bold block mt-1">{{ $message }}</span> @enderror
                    </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="@error('nisn') has-error @enderror">
                            ...
                            <input type="text" inputmode="numeric" wire:model="nisn" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm" {{ $jalur_pendaftaran != 'reguler' ? 'disabled' : '' }}>
                            @error('nisn') <span class="validation-error text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="@error('nik') has-error @enderror">
                            <label class="block text-sm font-bold text-unmaris-blue mb-1">NIK (KTP) *</label>
                            <input type="text" inputmode="numeric" maxlength="16" wire:model="nik" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm">
                            @error('nik') <span class="validation-error text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="@error('tempat_lahir') has-error @enderror">
                            ...
                            <input type="text" wire:model="tempat_lahir" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm">
                            @error('tempat_lahir') <span class="validation-error text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="@error('tgl_lahir') has-error @enderror">
                            ...
                            <input type="date" wire:model="tgl_lahir" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm">
                            @error('tgl_lahir') <span class="validation-error text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="@error('jenis_kelamin') has-error @enderror">
                            ...
                            <select wire:model="jenis_kelamin" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm">
                                <option value="">-- Pilih --</option>
                                <option value="L">Laki-laki</option>
                                <option value="P">Perempuan</option>
                            </select>
                            @error('jenis_kelamin') <span class="validation-error text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="@error('agama') has-error @enderror">
                            <label class="block text-sm font-bold text-unmaris-blue mb-1">Agama *</label>
                            <select wire:model="agama" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm">
                                <option value="">-- Pilih --</option>
                            ...
                            @error('agama') <span class="validation-error text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="md:col-span-2 @error('nomor_hp') has-error @enderror">
                            ...
                            <input type="tel" wire:model="nomor_hp" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm">
                            @error('nomor_hp') <span class="validation-error text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div class="md:col-span-2 @error('alamat') has-error @enderror">
                            <label class="block text-sm font-bold text-unmaris-blue mb-1">Alamat Lengkap (Sesuai KTP) *</label>
                            <textarea wire:model="alamat" rows="2" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm"></textarea>
                            @error('alamat') <span class="validation-error text-red-600 font-bold text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
            ...
            <div class="animate-fade-in-up space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2 @error('asal_sekolah') has-error @enderror">
                        ...
                        <input type="text" wire:model="asal_sekolah" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm">
                        @error('asal_sekolah') <span class="validation-error text-red-600 text-xs font-bold block">{{ $message }}</span> @enderror
                    </div>
                    <div class="@error('tahun_lulus') has-error @enderror">
                        ...
                        <input type="text" inputmode="numeric" maxlength="4" wire:model="tahun_lulus" class="w-full bg-gray-50 border-2 border-unmaris-blue rounded-lg py-3 px-4 font-medium text-unmaris-blue text-sm">
                        @error('tahun_lulus') <span class="validation-error text-red-600 text-xs font-bold block">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="md:col-span-2 bg-blue-50 p-4 border-2 border-unmaris-blue rounded-xl space-y-4">
                        <div class="@error('pilihan_prodi_1') has-error @enderror">
                            <label class="block text-sm font-black text-unmaris-blue mb-2 uppercase">Pilihan 1 (Utama) *</label>
                            ...
                            @error('pilihan_prodi_1') <span class="validation-error text-red-600 text-xs font-bold block mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div class="@error('pilihan_prodi_2') has-error @enderror">
                            <label class="block text-sm font-black text-unmaris-blue mb-2 uppercase">Pilihan 2 (Wajib) *</label>
        ...
        @if($currentStep == 3)
            <div x-data="{ agreed: false, uploading: false, progress: 0 }" 
                ...
                <!-- PROGRESS BAR GLOBAL -->
                <div x-show="uploading" class="fixed top-0 left-0 w-full z-50 bg-black/80 flex items-center justify-center h-full">
                    <div class="bg-white p-6 rounded-xl w-64 text-center">
                        <div class="mb-2 font-bold text-unmaris-blue animate-pulse">Mengunggah... <span x-text="progress + '%'"></span></div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2 bg-green-50 p-4 border-2 border-unmaris-blue border-dashed rounded-xl">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="@error('nama_ayah') has-error @enderror">
                                <label class="block text-xs font-bold text-gray-500 mb-1">Nama Ayah (Sesuai KK) *</label>
                            ...
                            <div class="@error('nama_ibu') has-error @enderror">
                                <label class="block text-xs font-bold text-gray-500 mb-1">Nama Ibu (Sesuai KK) *</label>
                    ...
                    <!-- Uploads -->
                    @if($jalur_pendaftaran == 'beasiswa')
                    <div class="md:col-span-2 bg-yellow-100 p-6 border-4 border-yellow-500 rounded-xl relative shadow-sm @error('file_beasiswa') has-error @enderror">
                        ...
                        <input type="file" wire:model="file_beasiswa" wire:key="beasiswa_input" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-yellow-50 file:text-yellow-700 hover:file:bg-yellow-100 transition"/>
                        @error('file_beasiswa') <span class="validation-error text-red-600 font-bold text-xs mt-1 block bg-red-50 p-1 border border-red-200 rounded text-center">{{ $message }}</span> @enderror
                    </div>
                    @endif
                    ...
                    <div class="bg-white border-2 border-unmaris-blue rounded-xl p-6 text-center shadow-neo relative group @error('file_ktp') has-error @enderror">
                        ...
                        <div class="mt-2 flex justify-center relative h-20 mb-4">
                            @if($file_ktp)
                                <div class="bg-green-100 text-green-700 p-2 rounded flex items-center gap-2 border border-green-300">
                                    <span class="text-xl">✅</span> <span class="text-xs font-bold truncate w-24">{{ $file_ktp->getClientOriginalName() }}</span>
                                </div>
                            @elseif($existingKtpPath)
                                <div class="bg-blue-100 text-blue-700 p-2 rounded flex items-center gap-2 border border-blue-300">
                                    <span class="text-xl">📂</span> <span class="text-xs font-bold">File Tersimpan</span>
                                </div>
                            @else
                        ...
                        <input type="file" wire:model="file_ktp" wire:key="ktp_input" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".pdf,.jpg,.jpeg,.png">
                        @error('file_ktp') <span class="validation-error text-red-600 font-bold text-xs block mt-2 bg-red-50 p-1 border border-red-200 rounded">{{ $message }}</span> @enderror
                    ...
                    <div class="md:col-span-2 bg-yellow-50 border-2 border-yellow-500 border-dashed rounded-xl p-6 text-center relative group">
                        ...
                        <div class="mt-2 flex justify-center relative h-12 mb-2">
                            ...
                            @elseif($existingAktaPath)
                                <span class="text-xs font-bold text-blue-600 bg-white px-2 py-1 rounded border border-blue-500">File Tersimpan</span>
                            @else
                                <span class="text-xs text-gray-500">Klik area ini untuk upload</span>
                            @endif
                        </div>
                        <input type="file" wire:model="file_akta" wire:key="akta_input" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".pdf,.jpg,.jpeg,.png">
                        @error('file_akta') <span class="validation-error text-red-600 font-bold text-xs block mt-2">{{ $message }}</span> @enderror
                    ...
                    <div class="md:col-span-2 bg-blue-50 border-4 border-unmaris-blue rounded-xl p-6 relative">
                        <div class="mb-4 @error('jenis_dokumen') has-error @enderror">
                            ...
                            <div class="flex gap-4">
                                <label class="flex items-center gap-2 bg-white px-4 py-2 rounded border hover:border-unmaris-blue"><input type="radio" wire:model.live="jenis_dokumen" value="ijazah"><span class="font-bold text-sm">Ijazah Asli</span></label>
                                <label class="flex items-center gap-2 bg-white px-4 py-2 rounded border hover:border-unmaris-blue"><input type="radio" wire:model.live="jenis_dokumen" value="skl"><span class="font-bold text-sm">SKL</span></label>
                            </div>
                            @error('jenis_dokumen') <span class="validation-error text-red-600 font-bold text-xs block mt-1">{{ $message }}</span> @enderror
                        </div>
                ...
                <div class="mt-8 p-5 rounded-xl border-4 transition-colors duration-300 shadow-neo"
                    ...
                    <label class="flex items-start gap-4 cursor-pointer group">
                        <div class="relative flex-shrink-0 mt-1">
                            <input type="checkbox" x-model="agreed" class="w-8 h-8 text-unmaris-blue border-4 border-black rounded focus:ring-0 cursor-pointer">
                            <span x-show="!agreed" class="animate-ping absolute inset-0 rounded-md bg-yellow-600 opacity-75"></span>
                        </div>
                        <div>
                            <div class="font-black text-xs uppercase tracking-widest mb-1" :class="agreed ? 'text-green-800' : 'text-red-500'"> <!-- Changed to red-500 -->
                                <span x-show="!agreed">⚠️ WAJIB DICENTANG</span>
                                <span x-show="agreed" style="display: none;">✅ TERIMA KASIH</span>
                            </div>
                            <span class="text-sm md:text-base font-bold text-gray-900 leading-tight">Saya menyatakan bahwa data ini benar.</span>
                        </div>
                    </label>
                </div>
    ...
    <!-- MODAL KONFIRMASI -->
    <div x-show="showConfirmModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"
        ...
        <div class="bg-white border-4 border-unmaris-blue rounded-3xl p-6 max-w-md w-full shadow-2xl text-center relative">
             <div class="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-yellow-400 border-4 border-black rounded-full p-4 shadow-neo text-4xl">🤔</div>
             <div class="mt-8">
                 ...
                 <div class="flex flex-col gap-3">
                     ...
                     <button wire:click="submit" wire:loading.attr="disabled" class="w-full bg-green-500 hover:bg-green-600 text-white font-black py-3 rounded-xl border-2 border-black shadow-neo hover:shadow-none transition-all uppercase flex justify-center items-center gap-2">
                        <span wire:loading.remove>✅ Ya, Kirim!</span>
                        <span wire:loading>Menyimpan... ⏳</span>
                     </button>
                     <button @click="showConfirmModal = false" class="w-full bg-white hover:bg-gray-100 text-gray-700 font-bold py-3 rounded-xl border-2 border-gray-300 transition-all uppercase">🔍 Cek Lagi</button>
                 </div>
             </div>
        </div>
    </div>
...
<script>
    document.addEventListener('livewire:initialized', () => {
        const scrollToError = () => {
            ...
            if (textError) {
                textError.scrollIntoView({ behavior: 'smooth', block: 'center' });
                return true;
            }
            return false;
        };
