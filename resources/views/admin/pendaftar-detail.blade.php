<x-admin-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-8">
            ...
            <!-- Pas Foto Card -->
            <div
                class="bg-white border-4 border-unmaris-blue shadow-neo rounded-xl p-6 text-center relative overflow-hidden">
                ...
                @else
                    <div
                        class="w-48 h-64 bg-gray-100 border-4 border-unmaris-blue border-dashed mx-auto flex flex-col items-center justify-center mb-4 text-gray-400">
                ...
                <div class="font-black text-xl text-unmaris-blue uppercase leading-tight">{{ $pendaftar->user->name }}
                </div>
            ...
            <div class="bg-unmaris-yellow border-4 border-unmaris-blue shadow-neo rounded-xl p-6">
                <h3
                    class="font-black text-unmaris-blue text-lg mb-4 uppercase border-b-2 border-unmaris-blue pb-2 flex items-center">
                    ⚡ Verifikasi Status
                </h3>
                ...
                <form action="{{ route('admin.pendaftar.update-status', $pendaftar->id) }}" method="POST"
                    ...
                    <div class="bg-white border-2 border-unmaris-blue p-3 rounded mb-4 text-center">
                        <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Status Saat
                            Ini</span>
                    ...
                    <button type="submit" name="status" value="verifikasi"
                        class="w-full bg-blue-100 hover:bg-blue-200 text-blue-900 border-2 border-unmaris-blue font-bold py-2 rounded shadow-sm hover:shadow-none transition-all text-sm flex justify-center items-center">
                        🔄 Sedang Diverifikasi
                    </button>

                    <button type="submit" name="status" value="lulus"
                        onclick="return confirm('Yakin nyatakan LULUS?')"
                        class="w-full bg-unmaris-green hover:bg-green-600 text-white border-2 border-unmaris-blue font-black py-3 rounded shadow-[2px_2px_0px_0px_#1E3A8A] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all flex justify-center items-center">
                        ✅ NYATAKAN LULUS
                    </button>

                    <button type="submit" name="status" value="gagal"
                        onclick="return confirm('Yakin nyatakan TIDAK LULUS?')"
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white border-4 border-unmaris-blue shadow-neo rounded-xl p-8 relative">
                ...
                <h3
                    class="font-black text-2xl text-unmaris-blue mb-6 border-b-4 border-unmaris-yellow inline-block pb-1">
                    📝 Biodata Lengkap
                </h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                    <div>
                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Jalur
                            Pendaftaran</label>
                        <div class="font-bold text-lg text-unmaris-blue">{{ ucfirst($pendaftar->jalur_pendaftaran) }}
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Tempat, Tanggal
                            Lahir</label>
                    ...
                    <div>
                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Kontak
                            (Email/HP)</label>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div
                            class="font-bold text-lg text-unmaris-blue bg-gray-50 p-3 rounded border-2 border-gray-100">
                            {{ $pendaftar->alamat }}
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white border-4 border-unmaris-blue shadow-neo rounded-xl p-8">
                <h3
                    class="font-black text-xl text-unmaris-blue mb-6 border-b-4 border-unmaris-yellow inline-block pb-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                    ...
                    <!-- Pilihan Prodi -->
                    <div class="md:col-span-2">
                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Pilihan Program
                            Studi</label>
                        <div class="flex flex-col gap-2 mt-1">
                            <div class="flex items-center bg-blue-50 border border-blue-100 p-2 rounded">
                                <span
                                    class="bg-unmaris-blue text-white text-xs font-black px-2 py-1 rounded mr-2">1</span>
                            ...
                            @if ($pendaftar->pilihan_prodi_2)
                                <div class="flex items-center bg-gray-50 border border-gray-100 p-2 rounded opacity-75">
                                    <span
                                        class="bg-gray-400 text-white text-xs font-black px-2 py-1 rounded mr-2">2</span>
                    ...
                    <div>
                        ...
                        <div class="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded inline-block mt-1">
                            {{ $pendaftar->pekerjaan_ayah ?? '-' }}</div>
                    </div>
                    <div>
                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Nama Ibu</label>
                        <div class="font-bold text-lg text-unmaris-blue">{{ $pendaftar->nama_ibu }}</div>
                        <div class="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded inline-block mt-1">
                            {{ $pendaftar->pekerjaan_ibu ?? '-' }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border-4 border-unmaris-blue shadow-neo rounded-xl p-8">
                ...
                <div
                    class="flex flex-col md:flex-row items-center justify-between bg-gray-50 border-2 border-unmaris-blue rounded-lg p-4 gap-4">
                    ...
                    @if ($pendaftar->ijazah_path)
                        <a href="{{ asset('storage/' . $pendaftar->ijazah_path) }}" target="_blank"
                            class="w-full md:w-auto text-center bg-unmaris-blue hover:bg-blue-800 text-white font-black py-3 px-6 rounded-lg border-2 border-black shadow-[2px_2px_0px_0px_#000] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all flex items-center justify-center gap-2">
                            <span>Buka File</span>
            ...
            <div class="bg-white border-4 border-unmaris-blue shadow-neo rounded-xl p-6 mt-6">
                ...
                <div class="text-center mb-4">
                    @if ($pendaftar->status_pembayaran == 'lunas')
                        <span
                            class="bg-green-100 text-green-800 border-2 border-green-600 px-3 py-1 rounded font-black uppercase text-sm">LUNAS</span>
                    @elseif($pendaftar->status_pembayaran == 'menunggu_verifikasi')
                        <span
                            class="bg-yellow-100 text-yellow-800 border-2 border-yellow-600 px-3 py-1 rounded font-black uppercase text-sm">BUTUH
                            CEK</span>
                    @else
                        <span
                            class="bg-red-100 text-red-800 border-2 border-red-600 px-3 py-1 rounded font-black uppercase text-sm">BELUM
                            BAYAR</span>
                    @endif
                </div>
                ...
                <div class="bg-blue-50 border-l-4 border-unmaris-blue p-3 mb-4 text-xs text-blue-900 rounded-r">
                    <strong>Info Admin:</strong> Jika Camaba melakukan pembayaran <u>Tunai di Kampus</u> (Loket
                    Keuangan), silakan terima uang/kwitansi lalu klik tombol <b>TERIMA (LUNAS)</b> di bawah secara
                    manual.
                </div>

                @if ($pendaftar->bukti_pembayaran)
                    <a href="{{ asset('storage/' . $pendaftar->bukti_pembayaran) }}" target="_blank"
                        class="block w-full text-center text-xs font-bold text-blue-600 underline mb-4 hover:text-blue-800">
                        📄 Lihat Bukti Transfer Online
                    </a>
                @else
                    <div class="text-center text-xs text-gray-400 mb-4 italic border-b border-gray-100 pb-2">
                        Tidak ada bukti transfer diunggah.<br>(Kemungkinan bayar tunai)
                    </div>
                @endif

                <!-- ACTION BUTTONS KHUSUS PAYMENT -->
                <div class="grid grid-cols-2 gap-2">
                    <form action="{{ route('admin.pendaftar.verify-payment', $pendaftar->id) }}" method="POST">
                        @csrf @method('PATCH')
                        <button type="submit" name="status_bayar" value="lunas"
                            class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-2 rounded text-xs border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px] transition-all">
                            TERIMA (LUNAS)
                        </button>
                    </form>
                    <form action="{{ route('admin.pendaftar.verify-payment', $pendaftar->id) }}" method="POST">
                        @csrf @method('PATCH')
                        <button type="submit" name="status_bayar" value="ditolak"
                            class="w-full bg-red-500 hover:bg-red-600 text-white font-bold py-2 rounded text-xs border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px] transition-all">
                            TOLAK / BATAL
                        </button>
                    </form>
                </div>
